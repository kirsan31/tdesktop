...
cd "%FullScriptPath%"
call gyp --depth=. --generator-output=.. -Goutput_dir=../out !BUILD_DEFINES! -Dofficial_build_target=%BuildTarget% -Dtdesktop_enable_sonnet_spellcheck=%TDESKTOP_ENABLE_SONNET_SPELLCHECK% Telegram.gyp --format=ninja
if %errorlevel% neq 0 goto error
call gyp --depth=. --generator-output=.. -Goutput_dir=../out !BUILD_DEFINES! -Dofficial_build_target=%BuildTarget% -Dtdesktop_enable_sonnet_spellcheck=%TDESKTOP_ENABLE_SONNET_SPELLCHECK% Telegram.gyp --format=msvs-ninja
if %errorlevel% neq 0 goto error
cd ../..
